...
namespace Client.Main
{
    public class MuGame : Game
    {
        ...
        protected override void Initialize()
        {
            ...
            ApplyLauncherPreferences(AppSettings);
        ...
        private static void ApplyLauncherPreferences(MuOnlineSettings settings)
        {
            var graphics = settings.Graphics;
            Constants.DISABLE_VSYNC = !graphics.VSync;
            Constants.UNLIMITED_FPS = graphics.FrameRateLimit <= 0;
            if (graphics.RenderScale is >= 0.375f and <= 3f)
            {
                Constants.RENDER_SCALE = graphics.RenderScale.Value;
            }

            if (graphics.Msaa.HasValue) Constants.MSAA_ENABLED = graphics.Msaa.Value;
            if (Enum.TryParse(graphics.ShadowQuality, true, out Constants.ShadowQuality shadowQuality))
                Constants.ApplyShadowQualityPreset(shadowQuality);
            Constants.DRAW_GRASS = graphics.Grass;
            Constants.ENABLE_DYNAMIC_LIGHTS = graphics.DynamicLights;
            Constants.ENABLE_WEAPON_TRAIL = graphics.WeaponTrails;
            Constants.HIGH_QUALITY_TEXTURES = graphics.HighQualityTextures;
            Constants.OPTIMIZE_FOR_INTEGRATED_GPU = graphics.OptimizeForIntegratedGpu;

            Constants.BACKGROUND_MUSIC = settings.Audio.MusicEnabled;
            Constants.BACKGROUND_MUSIC_VOLUME = Math.Clamp(settings.Audio.MusicVolume, 0, 100);
            Constants.SOUND_EFFECTS = settings.Audio.EffectsEnabled;
            Constants.SOUND_EFFECTS_VOLUME = Math.Clamp(settings.Audio.EffectsVolume, 0, 100);
        }
